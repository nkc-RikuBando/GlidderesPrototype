    ...
    namespace Director
    {
        public class GameDirector : MonoBehaviour
        {
            ...
            private void CommentTableActiveByTurn()
            {
                ...
                if (turnCount > 1)
                {
                    commentOutput.SetTableActive("ゲーム開始１", false);
                ...
                {
                    commentOutput.SetTableActive("ゲーム中盤１", true);
                    commentOutput.SetInterval(Comment.interval_normal, 2.0f);
                }
                else if (turnProgress >= 0.7)
                {
                    commentOutput.SetTableActive("ゲーム中盤１", false);
                    commentOutput.SetInterval(Comment.interval_normal, 2.0f);
                }
            ...
            {
                // スキルに関する内容は無効化する
                commentOutput.SetTableActive("攻撃ヒット汎用１", false, 2.0f);
                commentOutput.SetTableActive("バフ使用汎用１", false, 2.0f);
            }
